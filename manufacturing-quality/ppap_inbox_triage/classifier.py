...
def _classify_normalized_text(
    ...
    *,
    ...
    for element in PPAP_LEVEL_3_ELEMENTS:
        ...
        if source == "content" and best_score > 0:
            best_score = min(best_score * CONTENT_SCORE_BOOST, 1.2)
            if not best_pattern.startswith("content:"):
                best_pattern = f"content:{best_pattern}"
...
def classify_file(file: InboxFile, *, text_content: str | None = None) -> list[ElementMatch]:
    ...
    content_matches = _classify_normalized_text(
        file,
        normalized_content,
        source="content",
    ...
    return _merge_matches(filename_matches, content_matches)
